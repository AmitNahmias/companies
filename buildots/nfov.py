...
class NFOV():
    ...
    def _calcSphericaltoGnomonic(self, convertedScreenCoord):
        ...
        lon = self.cp[0] + np.arctan2(x * sin_c, rou * np.cos(
            self.cp[1]) * cos_c - y * np.sin(self.cp[1]) * sin_c)
    ...
    def _bilinear_interpolation(self, screen_coord):
        ...
        x2 = np.add(x0, np.ones(uf.shape).astype(
            int))  # coords of pixel to top right
        ...
        nfov = np.reshape(np.round(AA + BB + CC + DD).astype(np.uint8),
    ...
    def toNFOV(self, frame, center_point):
        ...
        self.cp = self._get_coord_rad(center_point=center_point,
                                      isCenterPt=True)
...
if __name__ == '__main__':
    ...
    center_point = np.array(
        [0.5, .5])  # camera center point (valid range [0,1])
